...
const API_BASE = process.env.REACT_APP_API_URL || (typeof window !== "undefined" ? "" : "http://localhost:8000");
...
export default function GeneAI() {
  const [genes, setGenes] = useState([{ name: "", phenotype: "", customName: "" }]);
  ...
  const addGene = () => setGenes([...genes, { name: "", phenotype: "", customName: "" }]);
  const removeGene = (i) => setGenes(genes.filter((_, idx) => idx !== i));
  const updateGene = (i, f, v) => { const u = [...genes]; u[i] = { ...u[i], [f]: v }; setGenes(u); };

  const getGeneName = (g) => (g.name === "Custom" && g.customName.trim()) ? g.customName.trim() : g.name;

  const analyze = async () => {
    setLoading(true); setError(null); setResults(null); setRiskLevel(null);
    const valid = genes
      .map(g => ({ ...g, name: getGeneName(g) }))
      .filter(g => g.name && g.phenotype);
  ...
  const reset = () => { setGenes([{ name: "", phenotype: "", customName: "" }]); setDrug(""); setDrugSearch(""); setResults(null); setError(null); setRiskLevel(null); };
    ...
    <div style={{ minHeight: "100vh", background: "#f0f0f0", fontFamily: "'Inter','Helvetica Neue',sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <nav style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          ...
          <a href="/demo" style={{ padding: "8px 16px", fontSize: "14px", fontWeight: 500, color: "#555", textDecoration: "none", borderRadius: "6px" }}>Demo</a>
        </nav>
      </div>
        ...
        <div style={{ padding: "28px" }}>
          ...
          {genes.map((g, i) => (
            <div key={i} style={{ marginBottom: "10px" }}>
              ...
              {g.name === "Custom" && (
                <input type="text" placeholder="Enter gene symbol (e.g. CYP2D6)" value={g.customName}
                  onChange={e => updateGene(i, "customName", e.target.value)} style={{ ...inp, marginTop: 4, fontSize: "14px" }} />
              )}
            </div>
          ))}
