/**
 * GeneAI Web API client
 * Set VITE_API_URL to your Modal deployment (e.g. https://your-workspace--geneai-api.modal.run)
 */
...
export async function healthCheck(): Promise<{ status: string; api: string; version: string }> {
  if (!BASE) return { status: 'offline', api: 'geneai', version: '1.0.0' };
